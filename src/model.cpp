...
double TGalacticLOSModel::dA_dmu(double DM) const {
	return rho_ISM_los(DM) * exp10(DM / 5.);
}
...
void TEBVSmoothing::calc_pct_smoothing(unsigned int nside,
	...
	double alpha = exp10(_alpha_coeff[0] * log_scale + _alpha_coeff[1]);
	double beta = exp10(_beta_coeff[0] * log_scale + _beta_coeff[1]);
